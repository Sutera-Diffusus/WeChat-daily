...
- 安装包：双击 `微语_0.1.4_x64-setup.exe`，安装后从开始菜单或桌面快捷方式启动“微语”；
- 便携包：完整解压 `微语-便携版-0.1.4-win-x64.zip`，只双击其中的 `wei-daily-desktop.exe`；
- 便携包中的 `wei-daily-backend.exe` 必须和主程序同目录，不能单独启动或拆出来；
- `127.0.0.1:8765` 是桌面主程序与内置后端之间的本机回环通道，用户不需要手动启动端口、Python 服务或浏览器页面。

便携包内还包含 `使用说明.md`、`SHA256SUMS.txt` 和 `version.txt`。如果提示端口被占用，请先关闭其它微语实例或旧的本地服务。
